...
import CybarTreasury from '../../build/CybarTreasury.json'
...
export async function treasuryFixture(provider: Web3Provider, [wallet, lottery]: Wallet[]): Promise<TreasuryFixture> {
    ...
    await factory.createPair(cybarToken.address, tokenB.address, overrides)
    await factory.createPair(tokenA.address, tokenB.address, overrides)
    const pairAddress0 = await factory.getPair(cybarToken.address, tokenA.address)
    const pairAddress1 = await factory.getPair(cybarToken.address, tokenB.address)
    const pairAddress2 = await factory.getPair(tokenA.address, tokenB.address)
